...
- **多 GPU / 多模型实例：后置的加法**。SQLite 协调（ADR 0022）天然多进程安全，predictor 按 gpu_id 构建；届时起 N worker 各占一卡各载模型，唯一缺件是"按 Session 粘住 worker"的路由（Session 态在 worker 内存）。触发条件 = 单卡排队成为体感瓶颈或显存吃紧。v1 无废弃。
